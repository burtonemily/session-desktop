...
test('Disappear after read', async() => {
  ...
  const controlMessage = 'has set messages to disappear 1 minute after they have been read'
  // Create Contact
  await createContact(windowA, windowB, userA, userB);
  // Click on conversation in linked device
  await clickOnTestIdWithText(windowC, 'module-conversation__user__profile-name', userB.userName);
  await clickOnTestIdWithText(windowA, 'conversation-options-avatar');
  await clickOnElement([windowA, 'data-testid', 'disappearing-messages']);
  await clickOnElement([windowA, 'data-testid', 'disappearing-after-read-option']);
  ...
  await clickOnElement([windowA, 'data-testid', 'disappear-time-1-minute-option']);
  ...
  await hasTextElementBeenDeleted(windowA, controlMessage);
  // Send message 
  ...
  await sleepFor(60000);
  await hasTextElementBeenDeleted(windowA, testMessage);
  // Check window B 
  await hasTextElementBeenDeleted(windowB, testMessage);
});
...
const [windowC] = await linkedDevice(userA.recoveryPhrase)
const testMessage = 'Testing disappearing messages timer is working correctly'
const controlMessage = 'has set messages to disappear 10 seconds after they have been sent'
// Create Contact
await createContact(windowA, windowB, userA, userB);
// Click on conversation in linked device
await clickOnTestIdWithText(windowC, 'module-conversation__user__profile-name', userB.userName);
...
await Promise.all([ 
  hasTextElementBeenDeleted(windowA, testMessage),
  hasTextElementBeenDeleted(windowB, testMessage),
  hasTextElementBeenDeleted(windowC, testMessage)
]);
});
...
test('Disappear after read groups', async () => {
  ...
  const controlMessageText = 'has set messages to disappear 10 seconds after they have been sent'
